...
users_list = [Main_UserDB(name = 'Ivanov', id = 10, password= '******'), Main_UserDB(name='Petrov', id = 11, password = '*******')]
...
@users_router.post("/api/users/{id}", response_model = Union[Main_User, New_Respons])
def create_person(item: Annotated[Main_User, Body(embed=True, description="Новый пользователь")]):
    user = Main_UserDB(name=item.name, id=item.id, password = coder_passwd(item.name))
    users_list.append(user)
    return user


@users_router.put("/api/users/{id}", response_model = Union[Main_User, New_Respons])
